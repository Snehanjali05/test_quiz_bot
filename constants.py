...
if __name__ == "__main__":
    ...
    responses = generate_bot_responses("", session)  # First message to get welcome message
    assert responses == [BOT_WELCOME_MESSAGE], f"Expected welcome message, got {responses}"
    print(responses)

    # Simulate answering questions
    responses = generate_bot_responses("A programming language", session)
    assert responses == [PYTHON_QUESTION_LIST[1]["question"]], f"Expected second question, got {responses}"
    print(responses)
   
    responses = generate_bot_responses("A collection of items", session)
    assert responses == [PYTHON_QUESTION_LIST[2]["question"]], f"Expected third question, got {responses}"
    print(responses)
   
    responses = generate_bot_responses("A collection of key-value pairs", session)
    assert "Quiz Completed!" in responses[0], f"Expected final result, got {responses}"
    print(responses)